...
class AdminControlCliArgumentsSuite extends KyuubiFunSuite with TestPrematureExit {
  ...
  test("test refresh config") {
    ...
    assert(opArgs5.cliConfig.adminConfigOpts.configType === LIMITED_USERS)
  ...
  test("test --help") {
    ...
    val helpString =
      s"""kyuubi $KYUUBI_VERSION
         ...
         |  <configType>             The valid config type can be one of the following: $HADOOP_CONF, $USER_DEFAULTS_CONF, $KUBERNETES_CONF, $UNLIMITED_USERS, $LIMITED_USERS.
         |
