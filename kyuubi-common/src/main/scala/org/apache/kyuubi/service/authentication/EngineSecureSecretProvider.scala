...
import org.apache.kyuubi.config.KyuubiConf.ENGINE_SECURE_ACCESS_SECRET_PROVIDER_CLASS
...
object EngineSecureSecretProvider {
  def create(conf: KyuubiConf): EngineSecureSecretProvider = {
    val providerClass = Class.forName(conf.get(ENGINE_SECURE_ACCESS_SECRET_PROVIDER_CLASS))
