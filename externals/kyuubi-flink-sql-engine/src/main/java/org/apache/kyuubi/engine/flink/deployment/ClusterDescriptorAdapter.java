    ...
    return bridgeClientRequest(
        ...
        clusterClient -> {
          try {
            ...
            LOG.error(
                String.format(
                    "Session: %s. Failed to fetch job status for job %s", sessionId, jobId),
                e);
            throw new RuntimeException("Failed to fetch job status for job " + jobId, e);
          }
        });
  }
  ...
  protected <R> R bridgeClientRequest(
    ...
    try (final ClusterDescriptor<ClusterID> clusterDescriptor =
      ...
      try (ClusterClient<ClusterID> clusterClient =
        ...
        LOG.error(
            String.format(
                "Session: %s, job: %s. Could not retrieve or create a cluster.", sessionId, jobId),
            e);
        throw new RuntimeException("Could not retrieve or create a cluster.", e);
      }
      ...
      throw new RuntimeException("Could not locate a cluster.", e);
    }
  }
