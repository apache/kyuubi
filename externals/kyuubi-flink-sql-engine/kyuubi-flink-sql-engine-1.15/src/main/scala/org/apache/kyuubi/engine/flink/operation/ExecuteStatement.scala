...
class ExecuteStatement(
  ...
  private def runQueryOperation(operation: QueryOperation): Unit = {
    ...
    try {
      ...
      val dataTypes = resultDescriptor.getResultSchema.getColumnDataTypes
          ...
          case TypedResult.ResultType.PAYLOAD =>
            (1 to result.getPayload).foreach { page =>
              if (rows.size < resultMaxRows) {
                val rowData = executor.retrieveResultPage(resultId, page)
                rows ++= convertToRow(rowData, dataTypes)
              } else {
                loop = false
              }
            }
  ...
  private def convertToRow(rowData: JavaList[RowData], dataTypes: JavaList[DataType]): List[Row] = {
    rowData.asScala.map(r => {
      val row = Row.withPositions(r.getRowKind, r.getArity)
      for (i <- 0 until r.getArity) {
        val dataType = dataTypes.get(i)
        dataType.getLogicalType match {
          case _: ArrayType =>
            row.setField(i, r.getArray(i))
          case _: BinaryType =>
            val v = r.getBinary(i)
            row.setField(i, new String(v))
          case _: BigIntType =>
          ...
          case _: MapType =>
            row.setField(i, r.getMap(i).toString)
          case _: DoubleType =>
