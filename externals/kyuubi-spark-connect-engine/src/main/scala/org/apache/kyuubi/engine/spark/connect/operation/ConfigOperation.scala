...
import org.apache.kyuubi.engine.spark.connect.grpc.proto.{ConfigRequest, ConfigResponse, KeyValue}
...
class ConfigOperation(
  ...
  override protected def runInternal(): Unit = {
    ...
    builder.setServerSideSessionId("")
    responseObserver.onNext(builder.build())
    responseObserver.onCompleted()
  }

  private def handleSet(
      operation: ConfigRequest.Set,
  ...
  private def handleGetWithDefault(
      operation: ConfigRequest.GetWithDefault,
      conf: RuntimeConfig): ConfigResponse.Builder = {
    val builder = ConfigResponse.newBuilder()
    operation.getPairsList.asScala.iterator.foreach { pair =>
  ...
  private def handleGetOption(
      operation: ConfigRequest.GetOption,
      conf: RuntimeConfig): ConfigResponse.Builder = {
    val builder = ConfigResponse.newBuilder()
    operation.getKeysList.asScala.iterator.foreach { key =>
  ...
  private def handleGetAll(
      operation: ConfigRequest.GetAll,
      conf: RuntimeConfig): ConfigResponse.Builder = {
    val builder = ConfigResponse.newBuilder()
  ...
  private def handleIsModifiable(
      operation: ConfigRequest.IsModifiable,
      conf: RuntimeConfig): ConfigResponse.Builder = {
    val builder = ConfigResponse.newBuilder()
    operation.getKeysList.asScala.iterator.foreach { key =>
  ...
  private def getWarning(key: String): Option[String] = {
    if (ConfigOperation.unsupportedConfigurations.contains(key)) {
      ...
      SQLConf.deprecatedSQLConfigs.get(key).map(_.comment)
    }
  }

}
object ConfigOperation {
  private val unsupportedConfigurations =
