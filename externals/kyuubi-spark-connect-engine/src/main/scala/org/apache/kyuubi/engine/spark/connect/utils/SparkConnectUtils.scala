 ...
 */
package org.apache.kyuubi.engine.spark.connect.utils

import java.nio.file.{Path, Paths}

object SparkConnectUtils {
  def concatenatePaths(basePath: Path, otherPath: Path): Path = {
  ...
  def concatenatePaths(basePath: Path, otherPath: String): Path = {
