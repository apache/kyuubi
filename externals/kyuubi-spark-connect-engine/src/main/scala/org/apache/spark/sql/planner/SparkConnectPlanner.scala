...
import org.apache.spark.connect.proto.{DataType => _, Expression => _, PythonUDF => _, _}
...
import org.apache.spark.sql.connect.common._
...
import org.apache.spark.sql.catalyst.plans.logical.{AppendColumns, CoGroup, CollectMetrics, Deduplicate, DeduplicateWithinWatermark, DeserializeToObject, FlatMapGroupsWithState, LocalRelation, LogicalGroupState, LogicalPlan, MapGroups, MapPartitions, Sample, SerializeFromObject, Sort, SubqueryAlias, TypedFilter, Unpivot, UnresolvedHint}
...
class SparkConnectPlanner(val grpcSession: GrpcSession) extends Logging {
  private def transformGrpcSession: SparkConnectSessionImpl =
    session.asInstanceOf[SparkConnectSessionImpl]

  private def session: SparkSession = transformGrpcSession.spark

  private def userId: String = transformGrpcSession.sessionKey.userId

  private def sessionId: String = transformGrpcSession.sessionKey.sessionId
...
//    val blockId = CacheId(userId, sessionId, rel.getHash)
