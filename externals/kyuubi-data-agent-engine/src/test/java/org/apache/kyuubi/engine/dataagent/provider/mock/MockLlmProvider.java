...
public class MockLlmProvider implements DataAgentProvider {
  ...
  public MockLlmProvider(KyuubiConf conf) {
    ...
    if (jdbcUrlOpt.isEmpty()) {
      throw new IllegalArgumentException("JDBC URL is required for MockLlmProvider");
    }
    this.dataSource = DataSourceFactory.create(jdbcUrlOpt.get());
    this.toolRegistry = new ToolRegistry();
    this.toolRegistry.register(new SqlQueryTool(dataSource));
  }
  ...
  private void runWithToolCall(String sql, Consumer<AgentEvent> onEvent) {
    ...
    onEvent.accept(new ToolCall(toolCallId, "sql_query", toolArgs));

    // Execute the tool
    String toolOutput =
        toolRegistry.executeTool("sql_query", "{\"sql\":\"" + escapeJson(sql) + "\"}");
    onEvent.accept(new ToolResult(toolCallId, "sql_query", toolOutput, false));
