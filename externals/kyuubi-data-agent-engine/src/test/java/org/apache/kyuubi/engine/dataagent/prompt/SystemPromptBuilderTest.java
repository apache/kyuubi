...
public class SystemPromptBuilderTest {
  ...
  @Test
  public void testUnknownDatasourceUsesGenericDialectSection() {
    String prompt = SystemPromptBuilder.create().datasource("postgresql").build();
    assertTrue(prompt.contains("Current SQL dialect: postgresql"));
    assertTrue(prompt.contains("postgresql SQL syntax"));
  }
