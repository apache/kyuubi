 ...
 *
 * <p><b>Design note — we deliberately do NOT use a SQL parser.</b> Big-data engines (Spark, Trino,
 * Hive, Flink, Impala, ClickHouse, ...) all extend ANSI SQL with non-standard syntax. Any general
 * parser (JSqlParser, Calcite, ...) will either fail on legitimate dialect-specific queries (false
 * rejection) or require a per-engine grammar (heavy dependency footprint, version drift). For the
 * data-agent use case, the SQL is generated by an LLM that has been told via the system prompt
 * which tool to use for which kind of statement, and modern LLMs essentially never make the mistake
 * of feeding an INSERT to a tool whose description says "read-only". A keyword whitelist on the
 * first significant token is a sufficient guard against accidents and clearly communicates intent
 * to the LLM via the rejection message — without dragging in a parser dependency we'd have to
 * maintain forever.
 *
...
final class SqlReadOnlyChecker {
   ...
   *   <li>{@code EXPLAIN} — query plan inspection (note: in Postgres {@code EXPLAIN ANALYZE}
   *       actually executes; we trust the LLM not to wrap a mutation in EXPLAIN ANALYZE for the
   *       read-only tool)
