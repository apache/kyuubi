 ...
 * {@link AgentContext#emit}, and the agent thread blocks until the client responds via {@link
...
public class ApprovalMiddleware implements AgentMiddleware {
  ...
  public ApprovalMiddleware(ToolRegistry toolRegistry) {
    this(toolRegistry, DEFAULT_TIMEOUT_SECONDS);
  }

  public ApprovalMiddleware(ToolRegistry toolRegistry, long timeoutSeconds) {
  ...
  /**
   * Cancel all pending approval requests. Should be called when the session is closed to unblock
   * any waiting agent threads.
   */
