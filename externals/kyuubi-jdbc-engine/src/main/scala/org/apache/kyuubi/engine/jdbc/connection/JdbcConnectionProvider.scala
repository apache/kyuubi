...
abstract class JdbcConnectionProvider extends Logging {

  val name: String = classOf[JdbcConnectionProvider].getSimpleName
  ...
  def getConnection(kyuubiConf: KyuubiConf): Connection = {
    ...
    val url = kyuubiConf.get(ENGINE_JDBC_CONNECTION_URL)
