...
class ClickHouseOperationWithEngineSuite extends ClickHouseOperationSuite with HiveJDBCTestHelper {
  ...
  test("clickhouse - JDBC ExecuteStatement operation should contain operationLog") {
    withSessionHandle { (client, handle) =>
