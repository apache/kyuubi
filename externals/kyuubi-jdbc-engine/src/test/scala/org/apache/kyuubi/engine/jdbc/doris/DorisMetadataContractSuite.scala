 ...
 *  PK / FK ops not exercised - Doris MySQL-protocol driver does not expose them.
 */
class DorisMetadataContractSuite extends WithDorisEngine with HiveJDBCTestHelper {

  override protected def jdbcUrl: String = jdbcConnectionUrl

  test("doris - getTables result has JDBC contract column names") {
  ...
  test("doris - getCatalogs/getSchemas/getTableTypes/getTypeInfo/getFunctions result " +
    "schema contract") {
    withJdbcStatement() { statement =>
  ...
  test("doris - getColumns result has JDBC contract column names and column shape") {
    withJdbcStatement() { statement =>
      val meta = statement.getConnection.getMetaData
      statement.execute("create database if not exists kyuubi_meta_db")
