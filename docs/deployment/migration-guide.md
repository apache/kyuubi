...
* Since Kyuubi 1.7, when metadata store operation failed, Kyuubi retry synchronously instead of asynchronously, and
  the max attempts is controlled by `kyuubi.metadata.request.retry.maxAttempts` instead of infinite attempts.
  To restore the previous behavior, set `kyuubi.metadata.request.retry.async.enabled` to `false`, and `kyuubi.metadata.request.retry.maxAttempts` to `2147483647`(Int.MaxValue).
