...
* Since Kyuubi 1.12, the configuration `spark.sql.kyuubi.hive.connector.dropTableWithPurge` is introduced by Kyuubi Spark Hive connector(KSHC) to control whether DROP TABLE command completely remove its data by skipping trash. The default value is false. To restore the legacy behavior, set it to true.
