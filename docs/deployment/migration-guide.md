...
* Since Kyuubi 1.7, the REST API of `Open(create) a session` will not contain parameters `protocolVersion`, `user` `password` and `IpAddr`. User and password should be set in `Authorization` of http request if needed.
