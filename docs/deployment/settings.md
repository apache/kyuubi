...
kyuubi.session.conf.profile|&lt;undefined&gt;|When kyuubi.session.conf.advisor=org.apache.kyuubi.session.FileSessionConfAdvisor, Specify a session level configuration file, which will be combined with default.conf to have an impact. The corresponding configuration file is `conf/kyuubi-session-<profile>.conf`|string|1.7.0
