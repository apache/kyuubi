...
kyuubi\.session\.name|<div style='width: 65pt;word-wrap: break-word;white-space: normal'>&lt;undefined&gt;</div>|<div style='width: 170pt;word-wrap: break-word;white-space: normal'>Session name is used to relate the history session which can help optimize. Please set this config using different value for workflows which are session level. This config is none means we can not find the history session.</div>|<div style='width: 30pt'>string</div>|<div style='width: 20pt'>1.4.0</div>
