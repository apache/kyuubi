...
kyuubi.operation.plan.only.output.style|plain|Configures the planOnly output style. The value can be 'plain' or 'json', andthe default value is 'plain'. This configuration supports only the output styles of the Spark engine|string|1.7.0
...
kyuubi.session.conf.advisor|&lt;undefined&gt;|A config advisor plugin for Kyuubi Server. This plugin can provide some custom configs for different users or session configs and overwrite the session configs before open a new session. This config value should be a subclass of 'org.apache.kyuubi.plugin.SessionConfAdvisor' which has a zero-arg constructor.|string|1.5.0
