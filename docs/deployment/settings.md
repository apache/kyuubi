...
kyuubi.engine.initialize.sql.type|SQL|When set kyuubi.engine.initialize.sql.type=PATH,you can use the hdfs file path  set ConfigEntry(key=kyuubi.engine.session.initialize.sql, defaultValue=, doc=SemiColon-separated list of SQL statements to be initialized in the newly created engine session before queries. This configuration can not be used in JDBC url due to the limitation of Beeline/JDBC driver., version=1.3.0, type=seq) and ConfigEntry(key=kyuubi.engine.initialize.sql, defaultValue=SHOW DATABASES, doc=SemiColon-separated list of SQL statements to be initialized in the newly created engine before queries. i.e. use `SHOW DATABASES` to eagerly active HiveClient. This configuration can not be used in JDBC url due to the limitation of Beeline/JDBC driver., version=1.2.0, type=seq)|string|1.8.0
