...
kyuubi.session.conf.expired.duration|PT10M|When FileSessionConfAdvisor is used,this configuration item defines the duration (in milliseconds) of `$KYUUBI_CONF_DIR/kyuubi-session-<profile>.conf` in the cache. After exceeding this value, the file will be reloaded into the cache. |duration|1.7.0
