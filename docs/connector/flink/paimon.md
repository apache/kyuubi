...
For the knowledge not mentioned in this article, you can obtain it from its [Official Documentation](https://paimon.apache.org/docs/master/).
```
...
3. `paimon-flink-<version>.jar` (example: `paimon-flink-1.20-0.8.1.jar`), which can be found in the [Apache Paimon Supported Engines Flink](https://paimon.apache.org/docs/master/flink/installation#choose-an-artifact)
...
```{warning}
Please mind the compatibility of different Apache Paimon and Flink versions,
...
Taking `Query Table` as an example:
...
Taking `Rescale Bucket` as an example:
