...
| spark.sql.adaptive.removeRebalanceShuffle.smallPartitionSize        | 96mb                                   | The advisory partition size below which a partition is considered small. In the large-data scenario, the rebalance shuffle is removed when the rebalance input has no data-reducing operator and its stage size is larger than `spark.sql.shuffle.partitions` * this value.                                                                          | 1.12.0 |
| spark.sql.adaptive.removeRebalanceShuffle.tolerableSmallFileNum     | 5                                      | The tolerable number of small output files for the small-data scenario. When the rebalance input has no data-expanding operator and its stage size is smaller than `spark.sql.adaptive.advisoryPartitionSizeInBytes` * this value, the rebalance shuffle is removed.                                                                                 | 1.12.0 |
