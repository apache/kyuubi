...
- stage level config isolation in AQE
  ...
  In addition, `spark.sql.adaptive.rebalancePartitionsAdvisoryPartitionSizeInBytes` controls the advisory partition size of the
  `RebalancePartitions` operator injected before writing. When set, it takes precedence over `spark.sql.finalStage.adaptive.advisoryPartitionSizeInBytes`.

  Kyuubi can also remove the rebalance shuffle at AQE time when the materialized rebalance input makes the shuffle not worthwhile.
  This is controlled by `spark.sql.adaptive.removeRebalanceShuffle.enabled` and only applies to a `RebalancePartitions` shuffle that
  has no partition expressions (i.e. `REBALANCE_PARTITIONS_BY_NONE`) and whose advisory partition size is larger than
  `spark.sql.adaptive.advisoryPartitionSizeInBytes`. It runs as an AQE runtime optimizer rule that rewrites the logical plan, and only
  the rebalance that directly feeds the write (through at most `Project` and local `Sort`) is considered. Two scenarios trigger the
  removal, evaluated on the materialized query stage size below the rebalance:
  1. Large data: when the rebalance input has no data-reducing operator and the stage size is larger than
  `spark.sql.shuffle.partitions * spark.sql.adaptive.removeRebalanceShuffle.smallPartitionSize`, the shuffle is removed.
  2. Small data: when the rebalance input has no data-expanding operator and the stage size is smaller than
  `spark.sql.adaptive.advisoryPartitionSizeInBytes * spark.sql.adaptive.removeRebalanceShuffle.tolerableSmallFileNum`, the shuffle is removed.
...
| spark.sql.optimizer.inferRebalanceAndSortOrdersWithCheapColumns.enabled | true                                   | When true and `spark.sql.optimizer.inferRebalanceAndSortOrders.enabled` is true, only infer the rebalance and sort columns when all inferred columns are cheap expressions, i.e. attributes, foldable values, or field extractions over cheap expressions. This avoids evaluating expensive expressions during the inferred shuffle and sort.        | 1.12.0 |
| spark.sql.optimizer.skipInferRebalanceAndSortOrders.enabled             | false                                  | When true and `spark.sql.optimizer.inferRebalanceAndSortOrders.enabled` is true, only infer the rebalance partition columns and skip inferring the sort orders. Skipping the sort avoids a local sort before writing when only the file layout from rebalance is wanted.                                                                             | 1.12.0 |
...
| spark.sql.adaptive.removeRebalanceShuffle.enabled                       | false                                  | When true, the rebalance shuffle injected before writing can be removed at AQE time if the materialized rebalance input makes the shuffle not worthwhile. Only takes effect on a `RebalancePartitions` shuffle that has no partition expressions and whose advisory partition size is larger than `spark.sql.adaptive.advisoryPartitionSizeInBytes`. | 1.12.0 |
| spark.sql.adaptive.removeRebalanceShuffle.smallPartitionSize            | 128mb                                  | The advisory partition size below which a partition is considered small. In the large-data scenario, the rebalance shuffle is removed when the rebalance input has no data-reducing operator and its stage size is larger than `spark.sql.shuffle.partitions` * this value.                                                                          | 1.12.0 |
| spark.sql.adaptive.removeRebalanceShuffle.tolerableSmallFileNum         | 3                                      | The tolerable number of small output files for the small-data scenario. When the rebalance input has no data-expanding operator and its stage size is smaller than `spark.sql.adaptive.advisoryPartitionSizeInBytes` * this value, the rebalance shuffle is removed.                                                                                 | 1.12.0 |
