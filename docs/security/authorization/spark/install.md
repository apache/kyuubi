...
With the `kyuubi-spark-authz_*.jar` and its transitive dependencies available or use only `kyuubi-spark-authz-bundle_*.jar` for spark runtime classpath, such as
