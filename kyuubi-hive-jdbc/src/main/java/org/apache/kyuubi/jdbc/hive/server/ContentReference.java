...
/**
 * Abstraction that encloses information about content being processed -- input source or output
 * target, streaming or not -- for the purpose of including pertinent information in location (see
 * {@link com.fasterxml.jackson.core.JsonLocation}) objections, most commonly to be printed out as
 * part of {@code Exception} messages.
 *
 * @since 2.13
 */
public class ContentReference
   ...
   *
   * <p>NOTE: As of 2.13 assume to contain Binary content, meaning that no content snippets will be
   * included.
   */
  protected static final ContentReference UNKNOWN_CONTENT = new ContentReference(false, null);

  /**
   * Include at most first 500 characters/bytes from contents; should be enough to give context, but
   * not cause unfortunate side effects in things like logs.
   *
   * @since 2.9
   */
  ...
  /**
   * For static content, indicates offset from the beginning of static array. {@code -1} if not in
   * use.
   */
  protected final int _offset;

  /**
   * For static content, indicates length of content in the static array. {@code -1} if not in use.
   */
  protected final int _length;

  /**
   * Marker flag to indicate whether included content is textual or not: this is taken to mean, by
   * default, that a snippet of content may be displayed for exception messages.
   */
  ...
  /**
   * Accessor for getting a placeholder for cases where actual content is not known (or is not
   * something that system wants to expose).
   *
   * @return Placeholder "unknown" (or "empty") instance to use instead of {@code null} reference
   */
  ...
  public static ContentReference construct(
      boolean isContentTextual, Object rawContent, int offset, int length) {
    return new ContentReference(isContentTextual, rawContent, offset, length);
  }

  /**
   * Factory method for legacy code to use for constructing instances to content about which only
   * minimal amount of information is available. Assumed not to contain textual content (no snippet
   * displayed).
   *
   * @param isContentTextual Is raw content assumed to have textual content ({@code true}) or binary
   *     ({@code false})
   * @param rawContent Underlying raw content access
   * @return Instance with minimal information about content (basically just raw content reference
   *     without offsets
   */
  ...
  /**
   * Internal accessor, overridable, used for checking length (in units in which content is counted,
   * either bytes or chars) to use for truncation (so as not to include full content for humongous
   * sources or targets)
   *
  ...
  /**
   * Method for constructing a "source description" when content represented by this reference is
   * read.
   *
