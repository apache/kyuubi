...
public class ServiceRecord implements Cloneable {
  ...
  /**
   * A type string which MUST be in the serialized json. This permits fast discarding of invalid
   * entries
   */
  ...
  /**
   * Create a service record with no ID, description or registration time. Endpoint lists are set to
   * empty lists.
   */
   ...
   * @return the value as a string, or <code>defval</code> if the value was not present
   */
