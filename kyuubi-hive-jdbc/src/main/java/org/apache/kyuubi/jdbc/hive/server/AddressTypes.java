...
public interface AddressTypes {
  ...
  /**
   * Path <code>/a/b/c</code> style: {@value}. The entire path is encoded in a single entry
   *
   * <pre>
   ...
   *
   * <p>These are provide as a 3 element tuple of: hostname, port and optionally path (depending on
   * the application)
   *
   * <p>A single element would be
   *
   * <pre>
   *   ["zk1","2181","/registry"]
   * </pre>
   ...
   * </pre>
   *
   * <p>the third element in each entry , the path, MUST be the same in each entry. A client reading
   * the addresses of an endpoint is free to pick any of the set, so they must be the same.
   */
