...
public class FastHiveDecimalImpl extends FastHiveDecimal {
   ...
   *
   * <p>We use 3 long words to store the 38 digits of fast decimals and and 3 integers for sign,
   * integer digit count, and scale.
   *
   * <p>The lower and middle long words store 16 decimal digits each; the high long word has 6
   * decimal digits; total 38 decimal digits.
   *
   * <p>We do not try and represent fast decimal value as an unsigned 128 bit binary number in 2
   * longs. There are several important reasons for this.
   *
   * <p>The effort to represent an unsigned 128 integer in 2 Java signed longs is very difficult,
   * error prone, hard to debug, and not worth the effort.
   *
   * <p>The focus here is on reusing memory (i.e. with HiveDecimalWritable) as often as possible.
   ...
   *
   * <p>In other words, we are focused on avoiding the poor performance of Java general immutable
   * objects.
   *
   * <p>Reducing memory size or being concerned about the memory size of using 3 longs vs. 2 longs
   * for 128 unsigned bits is not the focus here.
   *
   * <p>Besides focusing on reusing memory, storing a limited number (16) decimal digits in the
   * longs rather than compacting the value into all binary bits of 2 longs has a surprising
   * benefit.
   *
   * <p>One big part of implementing decimals turns out to be manipulating decimal digits.
   *
   * <p>For example, rounding a decimal involves trimming off lower digits or clearing lower digits.
   ...
   *
   * <p>For example, say we want to scale (round) down the fraction digits of a decimal.
   *
   * <p>final long divideFactor = powerOfTenTable[scaleDown]; final long multiplyFactor =
   * powerOfTenTable[LONGWORD_DECIMAL_DIGITS - scaleDown];
   *
   * <p>result0 = fast0 / divideFactor + ((fast1 % divideFactor) * multiplyFactor); result1 = fast1
   * / divideFactor + ((fast2 % divideFactor) * multiplyFactor); result2 = fast2 / divideFactor;
   *
   * <p>It also turns out to do addition and subtraction of decimals with different scales can
   * involve overlap using more than 3 long words. Manipulating extra words is a natural extension
   * of the existing techniques.
   *
   * <p>Why is the decimal digits representation easier to debug? You can see the decimal digits in
   * the 3 long words and do not have to convert binary words to decimal to see the value.
   *
   * <p>16 decimal digits for a long was choose so that an int can have 1/2 or 8 decimal digits
   * during multiplication of int half words so intermediate multiplication results do not overflow
   * a long. And, so addition overflow is well below the sign bit of a long.
   */
  ...
  private static final int MULTIPLER_INTWORD_DECIMAL =
      (int) powerOfTenTable[INTWORD_DECIMAL_DIGITS];
  ...
  private static final int HIGHWORD_DECIMAL_DIGITS =
      MAX_DECIMAL_DIGITS - TWO_X_LONGWORD_DECIMAL_DIGITS;
  ...
  private static final long FULL_MAX_HIGHWORD_DECIMAL =
      powerOfTenTable[
              MAX_DECIMAL_DIGITS * 2 - (FOUR_X_LONGWORD_DECIMAL_DIGITS + INTWORD_DECIMAL_DIGITS)]
          - 1;
  ...
  private static void doRaiseSetFromBytesInvalid(
      byte[] bytes, int offset, int length, FastHiveDecimal fastResult) {
   ...
   *
   * <p>Syntax: [+|-][integerPortion][.[fractionalDigits]][{E|e}[+|-]exponent] // Where at least one
   * integer or fractional // digit is required...
   *
   * <p>We handle too many fractional digits by doing rounding ROUND_HALF_UP.
   *
   * <p>NOTE: The fastSetFromBytes method requires the caller to pass a fastResult parameter has
   * been reset for better performance.
   *
   ...
   *
   * <p>Designed for BinarySortable serialization format that separates the sign and scale from the
   * raw digits.
   *
   * <p>NOTE: The fastSetFromDigitsOnlyBytesAndScale method requires the caller to pass a fastResult
  ...
  private static BigInteger doBigIntegerScaleDown(BigInteger unscaledValue, int scaleDown) {
    BigInteger[] quotientAndRemainder =
        unscaledValue.divideAndRemainder(BigInteger.TEN.pow(scaleDown));
   ...
   *
   * <p>NOTE: The fastSetFromBigDecimal method requires the caller to pass a fastResult parameter
   * has been reset for better performance.
   *
  ...
  public static boolean fastSetFromBigDecimal(
    ...
    if (!allowRounding) {
      if (bigDecimal.signum() != 0) {
        ...
        // System.out.println("FAST_SET_FROM_BIG_DECIMAL stripTrailingZerosScale " +
        // stripTrailingZerosScale);
    ...
    if (!fastSetFromBigInteger(
        bigDecimal.unscaledValue(), bigDecimal.scale(), bigDecimal.precision(), fastResult)) {
      return false;
    }
    return true;
  }
   ...
   *
   * <p>NOTE: The fastSetFromString method requires the caller to pass a fastResult parameter has
   * been reset for better performance.
   *
   ...
   *
   * <p>NOTE: The fastSetFromLong method requires the caller to pass a fastResult parameter has been
   * reset for better performance.
   *
   ...
   *
   * <p>NOTE: The fastSetFromLongAndScale method requires the caller to pass a fastResult parameter
   * has been reset for better performance.
   *
   ...
   *
   * <p>NOTE: The fastSetFromFloat method requires the caller to pass a fastResult parameter has
   * been reset for better performance.
   *
   ...
   *
   * <p>NOTE: The fastSetFromDouble method requires the caller to pass a fastResult parameter has
   * been reset for better performance.
   *
   ...
   *
   * <p>For efficiency, we assume that fastResult is fastReset. This method does not set the
   * fastScale field.
   *
   * <p>NOTE: The fastSetFromBigInteger method requires the caller to pass a fastResult parameter
   * has been reset for better performance.
   *
  ...
  public static boolean fastSetFromBigInteger(
    ...
    // System.out.println("FAST_SET_FROM_BIG_INTEGER adjusted bigInteger " + bigInteger + "
    // precision " + precision);

    int integerDigitCount = precision - scale;
    // System.out.println("FAST_SET_FROM_BIG_INTEGER integerDigitCount " + integerDigitCount + "
    // scale " + scale);
    int maxScale;
    ...
    if (scale > maxScale) {
      ...
      if (trimAwayCount > 1) {
        ...
        BigInteger bigIntegerThrowAwayBelowRoundDigitDivisor =
            BIG_INTEGER_TEN.pow(trimAwayCount - 1);
        bigInteger = bigInteger.divide(bigIntegerThrowAwayBelowRoundDigitDivisor);
      }
      // System.out.println("FAST_SET_FROM_BIG_INTEGER with round digit bigInteger " + bigInteger +
      // " length " + bigInteger.toString().length());

      BigInteger[] quotientAndRemainder = bigInteger.divideAndRemainder(BIG_INTEGER_TEN);
      // System.out.println("FAST_SET_FROM_BIG_INTEGER quotientAndRemainder " +
      // Arrays.toString(quotientAndRemainder));
      ...
      if (quotientAndRemainder[1].intValue() >= 5) {
        if (quotient.equals(BIG_INTEGER_MAX_DECIMAL)) {
          ...
          // System.out.println("FAST_SET_FROM_BIG_INTEGER reached here... scale " + scale + "
          // maxScale " + maxScale);
   ...
   *
   * <p>NOTE: The fastFractionPortion method requires the caller to pass a fastResult parameter has
   * been reset for better performance.
   *
  ...
  public static void fastFractionPortion(
    ...
    if (result0 == 0 && result1 == 0 && result2 == 0) {
      ...
      fastResult.fastSet(
          fastSignum, result0, result1, result2, /* fastIntegerDigitCount */ 0, fastScale);
    }
  }
   ...
   *
   * <p>The 3 binary words highWord, middleWord, and lowerWord form a large binary value:
   *
   * <p>highWord * 2^(M+L) + middleWord * 2^L + lowerWord.
   *
   * <p>Where L is the number of bits in the lower word; M is the number of bits in the middle word.
   * We let L and M be different to support the SerializationUtil serialization where the lower word
   * is 62 bits and the remaining words are 63 bits...
   *
   ...
   * quotient).
   *
   * <p>The fast decimal is longwords of 16 digits each and we need binary words of 2^N. Since we
   * are in decimal form, we have do work to get to convert to binary form.
   *
   * <p>We effectively need to produce on big binary value (i.e. greater than 64 bits since
   * HiveDecimal needs 128 bits of binary which Java does not provide primitive support for) from
   * the decimal long words and get the lower N binary bit remainder.
   *
   * <p>We could try and do decimal division by 2^N to get the (integer) quotient, multiply the
   * quotient by 2^N decimal, and finally do a decimal subtract that from the original decimal. The
   * resulting decimal can be used to easily get the binary remainder.
   *
   * <p>However, currently, we do not have fast decimal division.
   *
   * <p>The "trick" we do here is to remember from your Algebra in school than multiplication and
   * division are inverses of each other.
   *
   * <p>So instead of doing decimal division by 2^N we multiply by the inverse: 2^-N.
   *
   * <p>We produce 1 binary word (remainder) and a decimal quotient for the higher portion.
   *
   * @param dividendFast0 The input decimal that will produce a single binary word remainder and
   *     decimal quotient.
   * @param dividendFast1 second word
   * @param dividendFast2 third word
   * @param fastInverseConst the fast decimal inverse of 2^N = 2^-N
   * @param quotientIntegerWordNum the word in the inverse multiplication result to find the
   *     quotient integer decimal portion
   * @param quotientIntegerDigitNum the digit in the result to find the quotient integer decimal
   *     portion
   * @param fastMultiplierConst The fast decimal multiplier for converting the quotient integer to
   *     the larger number to subtract from the input decimal to get the remainder.
   * @param scratchLongs where to store the result remainder word (index 3) and result quotient
  ...
  public static boolean doDecimalToBinaryDivisionRemainder(
    ...
    if (!fastMultiply5x6HalfWords(
        ...
        dividendFast1,
        dividendFast2,
        fastInverseConst.fast0,
        fastInverseConst.fast1,
        fastInverseConst.fast2,
        scratchLongs)) {
      // Overflow.
      return false;
    }
    ...
    if (!fastMultiply5x6HalfWords(
        ...
        quotientFast1,
        quotientFast2,
        fastMultiplierConst.fast0,
        fastMultiplierConst.fast1,
        fastMultiplierConst.fast2,
        scratchLongs)) {
      return false;
    }
   ...
   *
   * <p>The 3 binary words will form a large binary value that is the unsigned unscaled decimal
   * value:
   *
   * <p>highWord * 2^(M+L) + middleWord * 2^L + lowerWord.
   *
   ...
   * are in decimal form, we have do work to get to convert to binary form.
   *
   * <p>See the comments for doDecimalToBinaryDivisionRemainder for details on the parameters.
   *
   * <p>The lowerWord is produced by calling doDecimalToBinaryDivisionRemainder. The quotient from
   ...
   *
   * <p>ORC uses those SerializationUtils methods for its serialization.
   *
   * <p>A scratch bytes array is necessary to do the binary to decimal conversion for better
   * performance. Pass a FAST_SCRATCH_BUFFER_LEN_SERIALIZATION_UTILS_READ byte array for
   * scratchBytes.
   *
   * @param inputStream the stream to read from
   * @param scale the scale of the number
   * @param scratchBytes An array for the binary to decimal conversion for better performance. Must
   *     have length of FAST_SCRATCH_BUFFER_LEN_SERIALIZATION_UTILS_READ.
   * @param fastResult an object to reuse
   ...
   * comments are:
   *
   * <p>Write the arbitrarily sized signed BigInteger in vint format.
   *
   * <p>Signed integers are encoded using the low bit as the sign bit using zigzag encoding.
   *
   * <p>Each byte uses the low 7 bits for data and the high bit for stop/continue.
   *
   * <p>Bytes are stored LSB first.
   *
   * <p>NOTE: SerializationUtils.writeBigInteger sometimes pads the result with extra zeroes due to
   * BigInteger.bitLength -- we do not emulate that. SerializationUtils.readBigInteger will produce
  ...
  public static void fastDeserialize64(
      final long inputDecimal64Long, final int inputScale, FastHiveDecimal fastResult) {
  ...
  public static long fastSerialize64(
      int scale, int fastSignum, long fast1, long fast0, int fastScale) {
   ...
   *
   * <p>Our bigIntegerBytes methods create bytes in this format, too.
   *
   * <p>This method is designed for high performance and does not create an actual BigInteger during
   * binary to decimal conversion.
   *
   * @param bytes the bytes to read from
  ...
  /**
   * When fastSetFromBigIntegerBytesAndScale can handle the input because it is too large, we fall
   * back to this.
   */
   ...
   * accepts).
   *
   * <p>Used by LazyBinary, Avro, and Parquet serialization.
   *
   * <p>Scratch objects necessary to do the decimal to binary conversion without actually creating a
   ...
   *
   * <p>Used by Avro and Parquet serialization.
   *
   * <p>This emulates the OldHiveDecimal setScale / OldHiveDecimal getInternalStorage() behavior.
   *
  ...
  public static int fastBigIntegerBytesScaled(
    ...
    // OldHiveDecimal setScale and OldHiveDecimalWritable internalStorage, we need to trailing
    // zeroes
    // here.
    //
    ...
    if (fastSignum == 0 || serializeScale == fastScale) {
      ...
      return fastBigIntegerBytesUnscaled(
          fastSignum, fastTemp.fast0, fastTemp.fast1, fastTemp.fast2, scratchLongs, buffer);
    } else {
  ...
  public static boolean fastIsByte(
    ...
    if (fastIntegerDigitCount < MAX_BYTE_DIGITS) {
      ...
      if (fastSignum == 1) {
        return (fastCompareTo(
                fastSignum, fast0, fast1, fast2, fastScale, FASTHIVEDECIMAL_MAX_BYTE_VALUE_PLUS_ONE)
            < 0);
      } else {
        return (fastCompareTo(
                fastSignum,
                ...
                fast2,
                fastScale,
                FASTHIVEDECIMAL_MIN_BYTE_VALUE_MINUS_ONE)
            > 0);
      }
    }
  }
   ...
   *
   * <p>NOTE: Fractional digits are ignored in the test since fastShortValueClip() will remove them
   * (round down).
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
    ...
    if (fastIntegerDigitCount < MAX_SHORT_DIGITS) {
      ...
      if (fastSignum == 1) {
        return (fastCompareTo(
                ...
                fast2,
                fastScale,
                FASTHIVEDECIMAL_MAX_SHORT_VALUE_PLUS_ONE)
            < 0);
      } else {
        return (fastCompareTo(
                fastSignum,
                ...
                fast2,
                fastScale,
                FASTHIVEDECIMAL_MIN_SHORT_VALUE_MINUS_ONE)
            > 0);
      }
    }
  }
   ...
   *
   * <p>NOTE: Fractional digits are ignored in the test since fastIntValueClip() will remove them
   * (round down).
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
  ...
  public static boolean fastIsInt(
    ...
    if (fastIntegerDigitCount < MAX_INT_DIGITS) {
      ...
      if (fastSignum == 1) {
        return (fastCompareTo(
                fastSignum, fast0, fast1, fast2, fastScale, FASTHIVEDECIMAL_MAX_INT_VALUE_PLUS_ONE)
            < 0);
      } else {
        return (fastCompareTo(
                fastSignum, fast0, fast1, fast2, fastScale, FASTHIVEDECIMAL_MIN_INT_VALUE_MINUS_ONE)
            > 0);
      }
    }
  }
   ...
   *
   * <p>NOTE: Fractional digits are ignored in the test since fastLongValueClip() will remove them
   * (round down).
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
  ...
  public static boolean fastIsLong(
    ...
    if (fastIntegerDigitCount < MAX_LONG_DIGITS) {
      ...
      if (fastSignum == 1) {
        return (fastCompareTo(
                fastSignum, fast0, fast1, fast2, fastScale, FASTHIVEDECIMAL_MAX_LONG_VALUE_PLUS_ONE)
            < 0);
      } else {
        return (fastCompareTo(
                fastSignum,
                ...
                fast2,
                fastScale,
                FASTHIVEDECIMAL_MIN_LONG_VALUE_MINUS_ONE)
            > 0);
      }
    }
  }
  ...
  public static long fastLongValueClip(
    ...
    if (fastScale == 0) {
      ...
      // Do first comparison as UNSIGNED.
      if (fastCompareTo(
              1, result0, result1, result2, /* fastScale */ 0, FASTHIVEDECIMAL_MAX_LONG_VALUE)
          <= 0) {
        if (fastSignum == 1) {
  ...
  public static float fastFloatValue(
    ...
    BigDecimal bigDecimal =
        fastBigDecimalValue(fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale);
  ...
  public static BigInteger fastBigIntegerValue(
    ...
    if (fastSerializationScale != -1) {
      return fastBigIntegerValueScaled(
          fastSignum,
          fast0,
          fast1,
          fast2,
      ...
      if (fast1 == 0) {
        ...
        result =
            BigInteger.valueOf(fast0)
                .add(BigInteger.valueOf(fast1).multiply(BIG_INTEGER_LONGWORD_MULTIPLIER));
      }
    } else {
      result =
          BigInteger.valueOf(fast0)
              .add(BigInteger.valueOf(fast1).multiply(BIG_INTEGER_LONGWORD_MULTIPLIER))
              .add(BigInteger.valueOf(fast2).multiply(BIG_INTEGER_LONGWORD_MULTIPLIER_2X));
    }
  ...
  public static BigInteger fastBigIntegerValueScaled(
    ...
    //
    // Since we are emulating old behavior and recommending the use of
   ...
   *
   * <p>NOTE: We are not representing our decimal as BigDecimal now as OldHiveDecimal did, so this
   * is now slower.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
  ...
  public static boolean fastEquals(
    ...
    return (leftFast0 == fastRight.fast0
        && leftFast1 == fastRight.fast1
        && leftFast2 == fastRight.fast2);
  }
  ...
  private static final int ZERO_NEW_FASTER_HASH_CODE =
      doCalculateNewFasterHashCode(0, 0, 0, 0, 0, 0);

  /**
   * Hash code based on (new) decimal representation.
   *
   * <p>Faster than fastHashCode().
   *
   * <p>Used by map join and other Hive internal purposes where performance is important.
   *
   * <p>IMPORTANT: See comments for fastHashCode(), too.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
   * @param fast1 word 1
   ...
   *
   * <p>We need this when the OldHiveDecimal hash code has been exposed and and written or affected
   * how data is written.
   *
   * <p>This method supports compatibility.
   *
   * <p>Examples: bucketing and the Hive hash() function.
   *
   * <p>NOTE: It is necessary to create a BigDecimal object and use its hash code, so this method is
   * slow.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
  ...
  public static boolean fastScaleByPowerOfTen(
      FastHiveDecimal fastDec, int power, FastHiveDecimal fastResult) {
    return fastScaleByPowerOfTen(
        fastDec.fastSignum,
        fastDec.fast0,
        fastDec.fast1,
        fastDec.fast2,
        fastDec.fastIntegerDigitCount,
        fastDec.fastScale,
        power,
        fastResult);
  }
  ...
  public static boolean fastRound(
      FastHiveDecimal fastDec, int newScale, int roundingMode, FastHiveDecimal fastResult) {
    return fastRound(
        fastDec.fastSignum,
        fastDec.fast0,
        fastDec.fast1,
  ...
  private static void doClearRoundIntegerPortionAndAddOne(
      long fast0, long fast1, long fast2, int absRoundPower, FastHiveDecimal fastResult) {
   ...
   *
   * <p>ceiling(12400.8302, -2) = 12500 // E.g. Positive case FAST_ROUND_CEILING rr rrrr
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
   * @param fast1 word 1
  ...
  public static boolean fastRoundIntegerUp(
    ...
    if (roundPower >= 0) {
      throw new IllegalArgumentException(
          "Expecting roundPower < 0 (roundPower " + roundPower + ")");
    }
    ...
    if (fastScale == 0) {
      ...
      doFastScaleDown(fast0, fast1, fast2, /* scaleDown */ fastScale, fastResult);
    }
    ...
    if (!isRoundPortionAllZeroes) {
      ...
      doClearRoundIntegerPortionAndAddOne(
          fastResult.fast0, fastResult.fast1, fastResult.fast2, absRoundPower, fastResult);
    }
   ...
   *
   * <p>The fraction being scaled away is thrown away.
   *
   * <p>The signum will be updated if the result is 0, otherwise the original sign is unchanged.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
   * @param fast1 word 1
   ...
   *
   * <p>When the fraction being scaled away is &gt;= 0.5, the add 1.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
   * @param fast1 word 1
   ...
   *
   * <p>When the fraction being scaled away is exactly 0.5, then round and add 1 only if aaa. When
   * fraction is not exactly 0.5, then if fraction &gt; 0.5 then add 1. Otherwise, throw away
   * fraction.
   *
   * <p>The signum will be updated if the result is 0, otherwise the original sign is unchanged.
   *
   ...
   *
   * <p>When the fraction being scaled away is non-zero, return false.
   *
   * <p>The signum will be updated if the result is 0, otherwise the original sign is unchanged.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   ...
   *
   * <p>When the fraction being scaled away is non-zero, the add 1.
   *
   * @param fastSignum the sign (-1, 0, or +1)
   * @param fast0 word 0 of the internal representation
   * @param fast1 word 1
  ...
  public static boolean fastRoundFractionalUp(
    ...
    if (scaleDown < 1 || scaleDown > HiveDecimal.MAX_SCALE) {
      throw new IllegalArgumentException(
          "Expecting scaleDown > 0 and scaleDown < "
  ...
  public static void doFastScaleDown(
      FastHiveDecimal fastDec, int scaleDown, FastHiveDecimal fastResult) {
   ...
   */
  public static void doFastScaleDown(
      long fast0, long fast1, long fast2, int scaleDown, FastHiveDecimal fastResult) {
  ...
  public static boolean fastScaleUp(
      FastHiveDecimal fastDec, int scaleUp, FastHiveDecimal fastResult) {
  ...
  public static boolean isAllZeroesBelow(
      int fastSignum, long fast0, long fast1, long fast2, int power) {
  ...
  public static int fastTrailingDecimalZeroCount(
      long fast0, long fast1, long fast2, int fastIntegerDigitCount, int fastScale) {
    ...
    final int middleLongwordDigits =
        Math.min(fastScale - LONGWORD_DECIMAL_DIGITS, LONGWORD_DECIMAL_DIGITS);
      ...
      case UPDATE_SCALE_DOWN:
        {
          ...
          if (!fastRoundFractionalHalfUp(
              fastSignum, fast0, fast1, fast2, fastScale - maxScale, fastResult)) {
            return false;
          }
  ...
  public static boolean doAddSameScaleSameSign(
      FastHiveDecimal fastLeft, FastHiveDecimal fastRight, FastHiveDecimal fastResult) {
  ...
  public static boolean doSubtractSameScaleNoUnderflow(
      ...
      FastHiveDecimal fastRight,
      FastHiveDecimal fastResult) {
    return doSubtractSameScaleNoUnderflow(
        resultSignum,
        fastLeft.fast0,
        fastLeft.fast1,
        fastLeft.fast2,
  ...
  private static boolean doFinishAddSubtractDifferentScale(
    ...
    if (precision > HiveDecimal.MAX_PRECISION) {
      ...
      if (!fastRoundFractionalHalfUp5Words(
          1, result0, result1, result2, result3, result4, scaleDown, fastResult)) {
        // Handle overflow precision issue.
        if (resultScale > 0) {
          if (!fastRoundFractionalHalfUp(
              fastResult.fastSignum,
  ...
  public static boolean fastAdd(
    ...
    return fastAdd(
        ...
        fastLeft.fast1,
        fastLeft.fast2,
        fastLeft.fastIntegerDigitCount,
        fastLeft.fastScale,
        fastRight.fastSignum,
        fastRight.fast0,
        fastRight.fast1,
   ...
   *
   * <p>NOTE: Scale Determination for Addition/Subtraction
   *
   * <p>One could take the Math.min of the scales and adjust the operand with the lower scale have a
   * scale = higher scale.
   *
   * <p>But this does not seem to work with decimals with widely varying scales as these:
   *
   * <p>598575157855521918987423259.94094 dec1 (int digits 27,scale 5) +
   * 0.0000000000006711991169422033 dec2 (int digits 0, scale 28)
   *
   * <p>Trying to make dec1 to have a scale of 28 (i.e. by adding trailing zeroes) would exceed
   * MAX_PRECISION (int digits 27 + 28 &gt; 38).
   *
   * <p>In this example we need to make sure we have enough integer digit room in the result to
   * handle dec1's digits. In order to maintain that, we will need to get rid of lower fractional
   * digits of dec2. But when do we do that?
   *
   * <p>OldHiveDecimal.add does the full arithmetic add with all the digits using BigDecimal and
   * then adjusts the result to fit in MAX_PRECISION, etc.
   *
   * <p>If we try to do pre-rounding dec2 it is problematic. We'd need to know if there is a carry
   * in the arithmetic in order to know at which scale to do the rounding. This gets complicated.
   *
   * <p>So, the simplest thing is to emulate what OldHiveDecimal does and do the full digit addition
  ...
  public static boolean fastSubtract(
    ...
    if (leftSignum == 0) {
      fastResult.fastSet(
          flippedDecSignum, rightFast0, rightFast1, rightFast2, rightIntegerDigitCount, rightScale);
      return true;
    }
  ...
  public static boolean fastMultiply5x5HalfWords(
      FastHiveDecimal fastLeft, FastHiveDecimal fastRight, FastHiveDecimal fastResult) {
    return fastMultiply5x5HalfWords(
        fastLeft.fast0,
        fastLeft.fast1,
        fastLeft.fast2,
        fastRight.fast0,
   ...
   *
   * <p>The caller is responsible checking for overflow within the highword and determining if scale
   * down appropriate.
   *
  ...
  public static boolean fastMultiplyFullInternal(
      FastHiveDecimal fastLeft, FastHiveDecimal fastRight, long[] result) {
    return fastMultiplyFullInternal(
        fastLeft.fast0,
        fastLeft.fast1,
        fastLeft.fast2,
        fastRight.fast0,
   ...
   * to the maximum possible 76 digits. The caller is responsible for scaling and rounding the
   * results back to 38 or fewer digits.
   *
   * <p>The caller is responsible for determining the signum.
   *
   * @param left0
   * @param left1
   * @param left2
  ...
  public static boolean fastMultiplyFullInternal(
    ...
    product =
        halfRight3 * halfLeft4 + halfRight4 * halfLeft3 + (product / MULTIPLER_INTWORD_DECIMAL);
  ...
  private static long doSingleWordQuotient(
      long leftFast0, long leftFast1, long leftFast2, long rightFast0, FastHiveDecimal fastResult) {
        ...
        if (k1 == 0) {
          ...
          remainderDigitCount =
              LONGWORD_DECIMAL_DIGITS
                  + LONGWORD_DECIMAL_DIGITS
                  - fastLongWordTrailingZeroCount(remainder1);
        } else {
          ...
          remainderDigitCount =
              TWO_X_LONGWORD_DECIMAL_DIGITS
                  + LONGWORD_DECIMAL_DIGITS
                  - fastLongWordTrailingZeroCount(remainder0);
        }
      }
    }
  ...
  private static boolean fastSingleWordDivision(
    ...
    long remainderSubexpr2 =
        doSingleWordQuotient(leftFast0, leftFast1, leftFast2, rightFast0, fastResult);
    ...
    int remainderDigitCount =
        doSingleWordRemainder(
            leftFast0, leftFast1, leftFast2, rightFast0, remainderSubexpr2, fastResult);
  ...
  public static String fastToFormatString(
    ...
    final int index =
        doFastToFormatBytes(
            ...
            formatScale,
            scratchBuffer);
    return new String(scratchBuffer, index, FAST_SCRATCH_BUFFER_LEN_TO_BYTES - index);
  }
  ...
  public static int doFastToFormatBytes(
    ...
    if (formatScale >= fastScale) {
      ...
      return doFastToBytes(
          ...
          fastTemp.fast1,
          fastTemp.fast2,
          fastTemp.fastIntegerDigitCount,
          fastTemp.fastScale,
          formatScale,
          scratchBuffer);
    }
  ...
  public static String fastToString(
    ...
    return doFastToString(
        fastSignum, fast0, fast1, fast2, fastIntegerDigitCount, fastScale, fastTrailingZeroesScale);
  }
  ...
  public static String fastToString(
    ...
    return doFastToString(
        ...
        fastIntegerDigitCount,
        fastScale,
        fastTrailingZeroesScale,
        scratchBuffer);
  }

  public static String fastToDigitsOnlyString(
      long fast0, long fast1, long fast2, int fastIntegerDigitCount) {
    byte[] scratchBuffer = new byte[FAST_SCRATCH_BUFFER_LEN_TO_BYTES];
    final int index =
        doFastToDigitsOnlyBytes(fast0, fast1, fast2, fastIntegerDigitCount, scratchBuffer);
    return new String(scratchBuffer, index, FAST_SCRATCH_BUFFER_LEN_TO_BYTES - index);
  }
  ...
  public static int fastToDigitsOnlyBytes(
      long fast0, long fast1, long fast2, int fastIntegerDigitCount, byte[] scratchBuffer) {
