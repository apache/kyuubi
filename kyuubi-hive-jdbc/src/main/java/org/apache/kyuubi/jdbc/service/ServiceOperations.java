...
public final class ServiceOperations {
   ...
   *
   * <p>The service state is checked <i>before</i> the operation begins. This process is <i>not</i>
   * thread safe.
   *
   * @param service a service that must be in the state {@link Service.STATE#NOTINITED}
   ...
   * thread safe.
   *
   * @param service a service that must be in the state {@link Service.STATE#INITED}
   * @throws RuntimeException on a state change failure
   * @throws IllegalStateException if the service is in the wrong state
   */
   ...
   * Stop a service.
   *
   * <p>Do nothing if the service is null or not in a state in which it can be/needs to be stopped.
   *
   * <p>The service state is checked <i>before</i> the operation begins. This process is <i>not</i>
   * thread safe.
   *
  ...
  /**
   * Stop a service; if it is null do nothing. Exceptions are caught and logged at warn level. (but
   * not Throwables). This operation is intended to be used in cleanup operations
   *
