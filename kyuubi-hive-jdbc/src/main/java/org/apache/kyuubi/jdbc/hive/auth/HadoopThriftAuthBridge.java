  ...
  public static class Server {
    ...
    protected Server(String keytabFile, String principalConf, String clientConf)
      ...
      try {
        ...
        kerberosName = SecurityUtil.getServerPrincipal(clientConf, "0.0.0.0");
    ...
    /**
     * Create a TSaslServerTransport.Factory that, upon connection of a client socket, negotiates a
     * Kerberized SASL transport.
     *
     * @param saslProps Map of SASL properties
     */
    public TSaslServerTransport.Factory createSaslServerTransportFactory(
      ...
      transFactory.addServerDefinition(
          ...
          names[0],
          names[1], // two parts of kerberos principal
          saslProps,
    ...
    /**
     * Wrap a TTransportFactory in such a way that, before processing any RPC, it assumes the
     * UserGroupInformation of the user authenticated by the SASL transport.
     */
    ...
    /**
     * Wrap a TProcessor in such a way that, before processing any RPC, it assumes the
     * UserGroupInformation of the user authenticated by the SASL transport.
     */
    ...
    static class SaslDigestCallbackHandler implements CallbackHandler {
      ...
      public void handle(Callback[] callbacks) throws InvalidToken, UnsupportedCallbackException {
        ...
        if (pc != null) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "SASL server DIGEST-MD5 callback: setting password "
                    + "for client: "
        ...
        if (ac != null) {
          ...
          if (ac.isAuthorized()) {
            if (LOG.isDebugEnabled()) {
              ...
              LOG.debug(
                  "SASL server DIGEST-MD5 callback: setting "
    ...
    /**
     * Processor that pulls the SaslServer object out of the transport, and assumes the remote
     * user's UGI before calling through to the original processor.
     *
     * <p>This is used on the server side to set the UGI for each specific call.
     */
    ...
    /**
     * A TransportFactory that wraps another one, but assumes a specified UGI before calling
     * through.
     *
