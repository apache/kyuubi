...
object SparkJVMQuakeConf {
  ...
  val DRIVER_JVM_QUAKE_ENABLED =
    ConfigBuilder("spark.driver.jvmQuake.enabled")
      .doc("when true, Spark driver will start jvm quake and begin detecting JVM exceptions")
      .version("1.10.0")
      .booleanConf
  ...
  val EXECUTOR_JVM_QUAKE_ENABLED =
    ConfigBuilder("spark.executor.jvmQuake.enabled")
      .doc("when true, Spark executor will start jvm quake and begin detecting JVM exceptions")
      .version("1.10.0")
      .booleanConf
      .createWithDefault(false)
  ...
  val JVM_QUAKE_KILL_THRESHOLD =
    ConfigBuilder("spark.jvmQuake.killThreshold")
      .doc("When the jvm quake reaches this threshold, " +
        "kill process and dump heap if dump is enabled")
      .version("1.10.0")
      .timeConf(TimeUnit.SECONDS)
  ...
  val JVM_QUAKE_RUN_TIME_WEIGHT =
    ConfigBuilder("spark.jvmQuake.runTimeWeight")
      .doc("Weight of run time, this configuration will affect the value of JVM Quake")
      .version("1.10.0")
      .doubleConf
