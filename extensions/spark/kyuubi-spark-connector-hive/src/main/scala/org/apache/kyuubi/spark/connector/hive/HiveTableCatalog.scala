...
class HiveTableCatalog(sparkSession: SparkSession)
  ...
  private val externalCatalogPool = ExternalCatalogPool.getOrCreate(sparkSession)
  ...
  lazy val externalCatalog: ExternalCatalogWithListener = {
    val externalCatalog = externalCatalogPool.take(Ticket(catalogName, sparkConf, hadoopConf))
