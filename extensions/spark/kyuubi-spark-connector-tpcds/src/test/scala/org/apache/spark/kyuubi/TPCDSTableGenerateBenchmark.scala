...
class TPCDSTableGenerateBenchmark extends KyuubiFunSuite with KyuubiBenchmarkBase {
  ...
  private def numRowsTableBenchmark(): Unit = {
    ...
    TPCDSSchemaUtils.BASE_TABLES
      ...
      .map(_.getName)
      .foreach { table =>
        benchmark.addCase(s"$table benchmark", 10) { _ =>
  ...
  private def allRowsTableBenchmark(): Unit = {
    val benchmark =
      new Benchmark(
        s"TPCDS table generates all rows benchmark",
        0,
        minTime = 10.seconds,
        output = output)
    TPCDSSchemaUtils.BASE_TABLES
      ...
      .foreach { table =>
        ...
        benchmark.addCase(s"$tableName benchmark", 10) { _ =>
          generateTable(tableName, TPCDSStatisticsUtils.numRows(table, scale))
        }
      }
    benchmark.run()
  }
