...
class CatalogTableOptionTableExtractor extends TableExtractor {
  override def apply(spark: SparkSession, v1: AnyRef): Option[Table] = {
    ...
    catalogTable.flatMap(getExtractor[CatalogTableTableExtractor].apply(spark, _))
  }
}
...
class ResolvedTableTableExtractor extends TableExtractor {
  override def apply(spark: SparkSession, v1: AnyRef): Option[Table] = {
    ...
    val catalog = getExtractor[CatalogPluginCatalogExtractor].apply(catalogVal)
    val identifier = invoke(v1, "identifier")
    val maybeTable = getExtractor[IdentifierTableExtractor].apply(spark, identifier)
    ...
    maybeV2Relation match {
      ...
      case Some(v2Relation) =>
        ...
        maybeIdentifier.flatMap { id =>
          val maybeTable = getExtractor[IdentifierTableExtractor].apply(spark, id)
...
class LogicalRelationTableExtractor extends TableExtractor {
  override def apply(spark: SparkSession, v1: AnyRef): Option[Table] = {
    ...
    maybeCatalogTable.flatMap { ct =>
      getExtractor[CatalogTableTableExtractor].apply(spark, ct)
    }
  }
}
