...
class RewriteCommandStrategy(spark: SparkSession) extends Strategy {
  ...
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = {
    val rewriter =
      TABLE_COMMAND_SPECS.get(plan.getClass.getName).flatMap(_.sparkPlanRewriter)
        .orElse(DB_COMMAND_SPECS.get(plan.getClass.getName).flatMap(_.sparkPlanRewriter))
        .flatMap(SparkPlanRewriter.sparkPlanRewriters.get)
