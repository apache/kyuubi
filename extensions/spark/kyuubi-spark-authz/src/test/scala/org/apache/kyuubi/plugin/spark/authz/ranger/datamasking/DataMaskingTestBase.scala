...
trait DataMaskingTestBase extends AnyFunSuite with SparkSessionProvider with BeforeAndAfterAll {
  ...
  private def setup(): Unit = {
    ...
    // NOTICE: `bob` has a row filter `key < 20`
    sql("INSERT INTO default.src " +
      "SELECT 10, 10, 'hello', '分布式计算', timestamp'2018-11-17 12:34:56', 'World'")
    sql("INSERT INTO default.src " +
  ...
  test("simple query with a user doesn't have mask rules") {
    checkAnswer("kent", "SELECT key FROM default.src order by key", Seq(Row(1),Row(10), Row(20), Row(30)))
  }
  ...
  test("star") {
    val result =
      Seq(Row(1, md5Hex("1"), "xxxxx", "worlx", Timestamp.valueOf("2018-01-01 00:00:00"), "Xorld"),
        Row(10, md5Hex("10"), "xxxxx", "分布式计x", Timestamp.valueOf("2018-01-01 00:00:00"), "Xorld"))
    checkAnswer("bob", "SELECT * FROM default.src order by key", result)
  }

  test("simple udf") {
    val result =
      Seq(Row(md5Hex("10"), "xxxxx", "分布式计x", Timestamp.valueOf("2018-01-01 00:00:00"), "Xorld"))
    checkAnswer(
      "bob",
  ...
  test("in subquery") {
    ...
    checkAnswer(
      "bob",
      "SELECT value1, value2, value3, value4, value5 FROM default.src a WHERE value2 in " +
        "(SELECT value2 as key FROM default.src) order by a.key",
      result)
  }
  ...
  test("create a unmasked table as select from a masked one") {
    withCleanTmpResources(Seq(("default.src2", "table"))) {
      ...
      checkAnswer("bob", "SELECT value1 FROM default.src2  order by value1", Seq(Row(md5Hex("1")), Row(md5Hex("10"))))
    }
  }
  ...
  test("insert into a unmasked table from a masked one") {
    withCleanTmpResources(Seq(("default.src2", "table"), ("default.src3", "table"))) {
      ...
      checkAnswer("bob", "SELECT value1 FROM default.src2 order by value1", Seq(Row(md5Hex("1")), Row(md5Hex("1")),
        Row(md5Hex("10")), Row(md5Hex("10"))))
      ...
      checkAnswer("bob", "SELECT value FROM default.src3 order by value", Seq(Row(md5Hex("1")), Row(md5Hex("1")),
        Row(md5Hex("10")), Row(md5Hex("10"))))
    }
  }
  ...
  test("self join on a masked table") {
    val s = "SELECT a.value1, b.value1 FROM default.src a" +
      " join default.src b on a.value1=b.value1 order by a.value1"
    checkAnswer("bob", s, Seq(Row(md5Hex("1"), md5Hex("1")),
      Row(md5Hex("10"), md5Hex("10"))))
  ...
  test("self join on a masked table and filter the masked column with masked value") {
    ...
    val s2 = "SELECT a.value1, b.value1 FROM default.src a" +
      " join default.src b on a.value1=b.value1" +
      s" where a.value2='xxxxx' and b.value2='xxxxx' order by a.value1"
  ...
  test("union an unmasked table") {
    val s = """
      ...
      """
    checkAnswer("bob", s, Seq(Row("1"), Row("10"), Row("2"), Row("3"),
      Row(md5Hex("1")), Row(md5Hex("10"))))
  }

  test("union a masked table") {
    val s = "SELECT a.value1 FROM default.src a union" +
      " (SELECT b.value1 FROM default.src b)"
    checkAnswer("bob", s, Seq(Row(md5Hex("1")), Row(md5Hex("10"))))
  }
  ...
  test("KYUUBI #3581: permanent view should lookup rule on itself not the raw table") {
    ...
    withCleanTmpResources(Seq(("default.perm_view", "view"))) {
      checkAnswer(
        "perm_view_user",
        "SELECT value1, value2 FROM default.src where key < 20 order by value1",
        Seq(Row(1, "hello"), Row(10, "hello")))
      checkAnswer(
        "perm_view_user",
        "SELECT value1, value2 FROM default.perm_view where key < 20 order by value1",
        Seq(Row(md5Hex("1"), "hello"), Row(md5Hex("10"), "hello")))
    }
  }
}
