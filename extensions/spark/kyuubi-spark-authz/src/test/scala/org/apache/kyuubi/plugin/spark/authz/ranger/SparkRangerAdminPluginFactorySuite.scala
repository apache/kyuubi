...
import org.apache.kyuubi.plugin.spark.authz.ranger.SparkRangerAdminPlugin.{defaultAppId, getOrCreate, serviceType}
...
class SparkRangerAdminPluginFactorySuite extends AnyFunSuite {
  // scalastyle:on
  test("get or create Ranger plugin by catalog name with Ranger 2.1+") {
    assume(isRanger21orGreater)

    val rangerPlugin1 = getOrCreate()
    ...
    val rangerPlugin2 = getOrCreate(Some(catalog2))
  ...
  test("same service name of Ranger plugin for catalogs with Ranger 2.0 and below") {
    assume(!isRanger21orGreater)
