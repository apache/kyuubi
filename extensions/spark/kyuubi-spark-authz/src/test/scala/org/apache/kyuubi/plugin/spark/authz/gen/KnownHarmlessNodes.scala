...
object KnownHarmlessNodes {
  ...
  // The Spark minors the plugin currently supports and tests per-profile; the baseline
  // every entry was reviewed against when the allowlist was introduced.
  private val spark3x = Seq("3.3", "3.4", "3.5")

  // Entries additionally exercised in deny mode under the spark-4.1 profile. 4.0 is
  // deliberately absent until something verifies it: a version joins an entry's list by
  // being tested or reviewed, never by interpolation.
  private val spark3xAnd41 = spark3x :+ "4.1"
