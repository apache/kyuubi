  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    if (conf.getConf(KyuubiSQLConf.REMOVE_CUSTOMIZE_REPARTITION_BEFORE_INSERT)) {
      plan resolveOperators {
        case i @ InsertIntoStatement(_, _, _, Repartition(_, _, child), _, _) =>
          i.copy(query = child)
        case i @ InsertIntoStatement(_, _, _, RepartitionByExpression(_, child, _), _, _) =>
          i.copy(query = child)
        case c @ CreateTable(_, _, Some(Repartition(_, _, child))) =>
          c.copy(query = Some(child))
        case c @ CreateTable(_, _, Some(RepartitionByExpression(_, child, _))) =>
          c.copy(query = Some(child))
      }
    } else {
      plan
