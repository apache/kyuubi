...
from pyhive.tests.dbapi_test_case import DBAPITestCase, with_cursor
...
class TestHive(unittest.TestCase, DBAPITestCase):
    ...
    def test_cancel(self, cursor):
        ...
        self.assertEqual(
            cursor.poll().operationState, ttypes.TOperationState.RUNNING_STATE
        )
        cursor.cancel()
        self.assertEqual(
            cursor.poll().operationState, ttypes.TOperationState.CANCELED_STATE
        )
    ...
    def test_newlines(self):
        ...
        orig = " \r\n \r \n "
        cursor.execute("SELECT %s FROM one_row", (orig,))
    ...
    def test_ldap_connection(self):
        ...
        orig_ldap = os.path.join(
            rootdir, "scripts", "conf", "hive", "hive-site-ldap.xml"
        )
    ...
    def test_invalid_ldap_config(self):
        ...
        self.assertRaisesRegexp(
            ValueError, "Password.*LDAP", lambda: hive.connect(_HOST, password="")
        )
        self.assertRaisesRegexp(
            ValueError, "Password.*LDAP", lambda: hive.connect(_HOST, auth="LDAP")
        )
    ...
    def test_invalid_kerberos_config(self):
        ...
        self.assertRaisesRegexp(
            ValueError,
            "kerberos_service_name.*KERBEROS",
    ...
    def test_custom_connection(self):
        rootdir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        orig_ldap = os.path.join(
            rootdir, "scripts", "conf", "hive", "hive-site-custom.xml"
        )
        orig_none = os.path.join(rootdir, "scripts", "conf", "hive", "hive-site.xml")
        des = os.path.join("/", "etc", "hive", "conf", "hive-site.xml")
        try:
