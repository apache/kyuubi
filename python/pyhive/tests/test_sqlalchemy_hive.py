...
from pyhive.tests.sqlalchemy_test_case import SqlAlchemyTestCase, with_engine_connection
...
class TestSqlAlchemyHive(unittest.TestCase, SqlAlchemyTestCase):
    ...
    def test_dotted_column_names_raw(self, engine, connection):
        ...
        row = (
            connection.execution_options(hive_raw_colnames=True)
            .execute(text("SELECT * FROM one_row"))
            .fetchone()
        )
    ...
    def test_lots_of_types(self, engine, connection):
        ...
        table = Table(
            "test_table",
            MetaData(schema="pyhive_test_database"),
            *cols,
        )
        ...
        self.assertEqual(
            row.hive_timestamp, datetime.datetime(1970, 1, 1, 0, 0, 2, 123000)
        )
