...
def reflect_table(
    engine, connection, table, include_columns, exclude_columns, resolve_fks
):
...
class SqlAlchemyTestCase(with_metaclass(abc.ABCMeta, object)):
    ...
    def test_one_row_complex_null(self, engine, connection):
        one_row_complex_null = Table(
            "one_row_complex_null", MetaData(), autoload_with=engine
        )
    ...
    def test_reflect_with_schema(self, engine, connection):
        dummy = Table(
            "dummy_table", MetaData(schema="pyhive_test_database"), autoload_with=engine
        )
    ...
    def test_reflect_partitions(self, engine, connection):
        ...
        self.assertEqual(
            repr(many_rows.indexes), repr({Index("partition", many_rows.c.b)})
        )
    ...
    def test_unicode(self, engine, connection):
        ...
        if sqlalchemy_version >= 1.4:
            returned_str = connection.execute(
                sqlalchemy.select(
                    expression.bindparam("好", unicode_str, type_=String())
                ).select_from(one_row)
            ).scalar()
        else:
            returned_str = connection.execute(
