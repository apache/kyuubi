...
class KyuubiSessionManagerSuite extends KyuubiFunSuite {
  ...
  test("[KYUUBI #7244] skip invalid batch metadata during recovery") {
    withSessionManager { sessionManager =>
      ...
      assert(sessions.map(_.handle.identifier.toString) === Seq(validBatchId))
      val invalidMetadata = sessionManager.getBatchMetadata(invalidBatchId)
      assert(invalidMetadata.map(_.state).contains(OperationState.ERROR.toString))
      assert(invalidMetadata.flatMap(_.engineError).exists(_.contains("spark.executor.memory")))
    }
  }
