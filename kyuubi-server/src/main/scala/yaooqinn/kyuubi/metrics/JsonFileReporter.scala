...
private[metrics] class JsonFileReporter(conf: SparkConf, registry: MetricRegistry)
  ...
  private lazy val jsonMapper = new ObjectMapper().registerModule(
  ...
  def start(): Unit = {
    timer.schedule(new TimerTask {
      ...
      override def run(): Unit = try {
        ...
        bw.write(json)
        bw.close()
        fs.setPermission(tmpPath, FsPermission.createImmutable(Integer.parseInt("755", 8).toShort))
        ...
        case NonFatal(e) =>
          error("Error writing metrics to json file" + path, e)
      } finally {
        if (bw != null) {
          try {
            bw.close()
