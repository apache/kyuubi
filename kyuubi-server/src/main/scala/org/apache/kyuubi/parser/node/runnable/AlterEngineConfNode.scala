...
case class AlterEngineConfNode(config: Map[String, String]) extends RunnableNode {
  override def name(): String = "Alter Engine Conf"

  override protected type R = Unit

  override def execute(session: KyuubiSessionImpl): Unit = {
    KillEngineNode().execute(session)
    val conf = session.kyuubiConf
    config.map {
