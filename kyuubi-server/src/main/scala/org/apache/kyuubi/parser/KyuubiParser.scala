...
import org.apache.kyuubi.parser.node.LogicalNode
...
class KyuubiParser {
  ...
  def parsePlan(sqlText: String): LogicalNode = parse(sqlText) { parser =>
    astBuilder.visit(parser.singleStatement) match {
      case plan: LogicalNode => plan
      case _ =>
        throw new ParseException(s"Unsupported SQL statement $sqlText")
    }
  }
  ...
  protected def parse[T](command: String)(toResult: KyuubiSqlBaseParser => T): T = {
    ...
    try {
      ...
      case NonFatal(t) =>
        throw new ParseException(t.getMessage, Some(t))
    }
  }
}
