...
import org.apache.kyuubi.operation.BatchAppSubmissionState.BatchAppSubmissionState
    ...
    try {
      ...
      if (applicationFailed(applicationStatus)) {
        applicationStatus.get.get(ApplicationOperation.APP_STATE_KEY) match {
          case Some("KILLED") => submissionState = BatchAppSubmissionState.KILLED
          case _ => submissionState = BatchAppSubmissionState.FAILED
        }
