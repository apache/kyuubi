...
class BatchJobSubmission(
  ...
  private val applicationDestroyTimeout =
    session.sessionConf.get(KyuubiConf.BATCH_APPLICATION_DESTROY_TIMEOUT)
  ...
  private def submitAndMonitorBatchJob(): Unit = {
    ...
    try {
      ...
      if (applicationFailed(_applicationInfo)) {
        if (Utils.terminateProcess(process, applicationDestroyTimeout).isEmpty) {
          warn(s"batch job process does not exit after $applicationDestroyTimeout ms, " +
            "try to forcibly kill. Staging files generated by the process may be retained!")
        }
