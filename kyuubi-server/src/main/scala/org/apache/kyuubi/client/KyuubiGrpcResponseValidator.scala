...
import org.apache.kyuubi.util.{CloseableIterator, WrappedCloseableIterator}
...
trait KyuubiGrpcResponseValidator extends Logging {
  ...
  def verifyResponse[RespT <: GeneratedMessageV3](fn: => RespT): RespT = {
    ...
    if (field != null && response.hasField(field)) {
      ...
      debug("Server side session ID field not found in response - Ignoring.")
    }
    response
  }
