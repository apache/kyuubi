...
import org.apache.kyuubi.config.KyuubiConf.{FRONTEND_REST_BIND_HOST, FRONTEND_REST_BIND_PORT, METADATA_STORE_SESSIONS_RECOVERY_NUM_THREADS}
...
class KyuubiRestFrontendService(override val serverable: Serverable)
  ...
  private[kyuubi] def recoverBatchSessions(): Unit = {
    val recoveryNumThreads = conf.get(METADATA_STORE_SESSIONS_RECOVERY_NUM_THREADS)
