...
class MySQLNativePassword {
  final val pluginData = new PluginData

  def login(user: String, host: String, authResp: Array[Byte], database: String):
  ...
  private def isPasswordRight(password: String, authentication: Array[Byte]) =
