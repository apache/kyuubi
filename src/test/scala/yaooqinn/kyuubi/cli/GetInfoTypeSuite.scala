...
class GetInfoTypeSuite extends SparkFunSuite {
  ...
  test("Get Info Type basic tests") {
    assert(GetInfoType.getGetInfoType(TGetInfoType.CLI_DBMS_NAME) === DBMSName)
    assert(GetInfoType.getGetInfoType(TGetInfoType.CLI_DBMS_VER) === DBMSVesion)
    assert(GetInfoType.getGetInfoType(TGetInfoType.CLI_SERVER_NAME) === ServerName)
